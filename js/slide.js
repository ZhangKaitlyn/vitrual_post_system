...
var myFocus = {
  ...
  move: function(obj, prop, val, type, spd, fn) { //运动函数，spd为运动需要的时间，时间越大速度越小
    // console.log(obj+" "+prop+" "+val+" "+type+" "+spd+" "+fn);
    console.log(val);
    var t = 0,
  ...
  mF_liuzg: function(par) {
    // console.log(par);
    var box = this.$(par.id),
      boxH = box.childNodes[1].offsetHeight,
      t = par.time * 1000;
      // console.log(boxH);
    ...
    for (var i = 0; i < c; i++) { //初始化样式设置
      ...
      // console.log(this.$$('p', pic[i])[0].style.height);
    }
    ...
    var run = function(idx) {
      ...
      for (var i = 0; i < c; i++) {
        ...
        myFocus.move(myFocus.$$('p', pic[i])[0], 'top', -N * c * h - i * h+c, 'easeOut', spd);
...
myFocus.setting({
  ...
  id: 'myFocus', //焦点图ID
  chip: 4, //图片切片数量，能被焦点图的高整除才有效，默认为8片
